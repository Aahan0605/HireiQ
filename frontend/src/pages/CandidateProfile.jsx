...
export default function CandidateProfile() {
  ...
  // Recruiter Copilot States
  const [copilotType, setCopilotType] = useState('');
...
HireIQ Hiring Team`
  ...
  const handleGenerateCopilot = async (type) => {
    setCopilotLoading(true);
    setCopilotType(type);
    try {
      const res = await apiFetch(`${API}/features/copilot/generate`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ candidate_id: id, type })
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.detail || "Failed to generate copilot copy.");
      setCopilotOutput({ ...data, type });
      toast.success("Recruitment copy generated by AI!");
    } catch (err) {
  ...
  const handleStartInterview = async () => {
    setChatLoading(true);
    try {
      const res = await apiFetch(`${API}/features/interview/simulate`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ candidate_id: id, messages: [] })
      });
      const data = await res.json();
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      ...
      const res = await apiFetch(`${API}/features/interview/simulate`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ candidate_id: id, messages: payloadMessages })
      });
      const data = await res.json();
      ...
      if (data.is_completed) {
        ...
        setInterviewScorecard(data.scorecard);
        toast.success("AI Interview Simulation completed! Scorecard generated.");
      }
    } catch (err) {
      toast.error(err.message || "Error in chatbot reply.");
    } finally {
      setChatLoading(false);
    }
  };
  ...
  const handleCheckResume = async () => {
    ...
    try {
      const res = await apiFetch(`${API}/candidates/${id}/resume`);
      if (!res.ok) {
        const err = await res.json().catch(() => ({}));
        toast.error(err.detail || 'Resume file not available. Please re-upload.');
        setResumeTab('text');
        return;
      }
          ...
          <motion.div variants={fadeUp} className="flex flex-col gap-6 lg:col-span-2">
            ...
            <MagneticCard className="p-8 border-black/10 dark:border-white/10 bg-[#13131f]">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4 border-b border-white/5 pb-4">
                <div>
                  <h3 className="text-xl font-semibold text-white flex items-center gap-2">
                    💬 AI Interview Agent Simulation
                  </h3>
                  <p className="text-xs text-gray-500">Run a simulated first-round adaptive technical screening session with candidate</p>
                </div>
                {chatMessages.length === 0 && (
                  <button
                    onClick={handleStartInterview}
                    disabled={chatLoading}
                    className="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-semibold rounded-xl transition-all active:scale-95 disabled:opacity-50"
                  >
                    Start Simulation
                  </button>
                )}
              ...
              {chatMessages.length > 0 && (
                <div className="space-y-4">
                  {/* Chat Console */}
                  <div className="border border-white/5 bg-black/20 rounded-xl p-4 h-64 overflow-y-auto flex flex-col gap-3 scrollbar-thin">
                      ...
                      <div
                        ...
                        className={`flex flex-col max-w-[85%] rounded-xl p-3 text-xs leading-relaxed ${
                          msg.role === 'assistant'
                            ...
                            : 'bg-violet/10 border border-violet/20 text-white self-end'
                        }`}
                      >
                        <span className="font-bold text-[9px] uppercase tracking-wider mb-1 text-gray-400">
                          {msg.role === 'assistant' ? 'AI Recruiter' : (blindReview ? 'Candidate' : candidate?.name)}
                        </span>
                        <p className="whitespace-pre-wrap">{msg.content}</p>
                      </div>
                    ))}
                    {chatLoading && (
                      <div className="text-xs text-gray-500 flex items-center gap-1.5 self-start bg-white/5 p-2 rounded-xl">
                  ...
                  {!interviewCompleted && (
                    <form onSubmit={handleSendMessage} className="flex gap-2">
                      <input
                        ...
                        disabled={chatLoading}
                        className="flex-1 px-3 py-2 bg-white/5 border border-white/10 rounded-xl text-gray-300 text-xs outline-none focus:border-violet/50"
                      />
                      <button
                        type="submit"
                        disabled={chatLoading || !chatInput.trim()}
                        className="px-4 py-2 bg-violet hover:bg-violet-700 disabled:opacity-50 text-white text-xs font-semibold rounded-xl transition-all active:scale-95"
                      >
                        Send
                      </button>
                    </form>
                  ...
                  {/* Scorecard Display */}
                  {interviewScorecard && (
                    <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-xl p-5 space-y-4 mt-4">
                      <h4 className="text-sm font-bold text-emerald-400 flex items-center gap-1.5">
                        🏆 AI Screening Scorecard
                      </h4>
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                        <div className="bg-black/30 p-3 rounded-lg border border-white/5 text-center">
                          <span className="text-[10px] text-gray-400 block mb-1">Problem Solving</span>
                          <span className="text-base font-bold text-white">{interviewScorecard.problem_solving}/100</span>
                        </div>
                        <div className="bg-black/30 p-3 rounded-lg border border-white/5 text-center">
                          <span className="text-[10px] text-gray-400 block mb-1">Technical Depth</span>
                          <span className="text-base font-bold text-white">{interviewScorecard.technical_depth}/100</span>
                        </div>
                        <div className="bg-black/30 p-3 rounded-lg border border-white/5 text-center">
                          <span className="text-[10px] text-gray-400 block mb-1">Communication</span>
                          <span className="text-base font-bold text-white">{interviewScorecard.communication}/100</span>
                        </div>
                      </div>
                      <div className="pt-2 border-t border-white/5 text-xs text-gray-300">
                        <span className="text-gray-400 font-semibold block mb-1">Hiring Recommendation:</span>
                        <span className="px-2 py-0.5 rounded bg-emerald-500/20 text-emerald-400 font-bold border border-emerald-500/30">
                          {interviewScorecard.overall_recommendation}
                        </span>
                      </div>
                      <div className="text-xs text-gray-300 leading-relaxed font-sans">
                        <span className="text-gray-400 font-semibold block mb-0.5">Evaluation Summary:</span>
                        {interviewScorecard.notes}
                      </div>
                    </div>
                  )}
            ...
            {/* Email History timeline (Gmail/Outlook sync logs) */}
            <MagneticCard className="p-8 border-black/10 dark:border-white/10 bg-[#13131f]">
              <div className="flex items-center justify-between mb-4 border-b border-white/5 pb-4">
                <div>
                  <h3 className="text-xl font-semibold text-white">Gmail & Outlook Communications</h3>
                  <p className="text-xs text-gray-500">Sync history tracking communications with the candidate</p>
                </div>
                ...
                {resumeBlobUrl && (
                  <div className="flex gap-2">
                    ...
                    <a
                      ...
                      className="px-3 py-1.5 rounded-lg text-xs font-semibold bg-violet/10 text-violet border border-violet/20 hover:bg-violet/20 transition-colors"
                    >
                      ⬇ Download
                    </a>
                  </div>
                  ...
                  resumeBlobUrl ? (
                    ...
                    <div className="flex-1 flex items-center justify-center">
                      <div className="text-center space-y-3">
                        ...
                        <p className="text-gray-400 text-sm">Loading resume PDF...</p>
                      </div>
                    </div>
                  )
                ) : (
