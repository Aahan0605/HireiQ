...
def to_hireiq_features(json_resume: JSONResume) -> dict:
    """
    Converts JSONResume to HireIQ's existing feature dict format.
    """
    # 1. Basics Info
    basics = json_resume.basics
    name = basics.name if basics else ""
    email = basics.email if basics else ""
    phone = basics.phone if basics else ""
    
    # Location
    location_str = ""
    if basics and basics.location:
        loc = basics.location
        parts = [getattr(loc, field) for field in ['city', 'region', 'countryCode'] if getattr(loc, field)]
    ...
    if json_resume.skills:
        for skill in json_resume.skills:
            ...
            if skill.keywords:
                skills_list.extend(skill.keywords)
    skills_list = list(set(filter(None, skills_list)))
    ...
    if json_resume.work:
        ...
        def parse_date(date_str: str) -> float | None:
            ...
            if date_str in ("present", "current", "now", "still working", ""):
            ...
            # Fallback to general date regex/parsing
        ...
        for job in json_resume.work:
            t_start = parse_date(job.startDate)
            t_end = parse_date(job.endDate)
    ...
    if json_resume.education:
        for e in json_resume.education:
            parts = []
            if e.studyType:
                parts.append(e.studyType)
            if e.area:
                parts.append(e.area)
            if e.institution:
    ...
    # 5. Certifications
    certs_list = []
    if json_resume.certificates:
        for cert in json_resume.certificates:
            if cert.name:
                certs_list.append(cert.name)

    # 6. Projects
    projects_list = []
    if json_resume.projects:
        for proj in json_resume.projects:
            if proj.name:
                projects_list.append(proj.name)

    # 7. Github and Linkedin
    github_username = ""
    linkedin_url = ""
    if basics and basics.profiles:
        for p in basics.profiles:
            net = p.network.lower() if p.network else ""
            p_url = p.url.lower() if p.url else ""
            
            if net == "github" or "github.com" in p_url:
